...
private func returnToDeck(cardId: String, game: inout Game) {
    if let idx = game.player.remainingDeck.firstIndex(where: { $0.card.id == cardId }) {
        ...
        game.player.pendingExtraDeckCardIds[cardId, default: 0] += 1
    }
}
...
public final class GameController {
    ...
    private func resolveExtraDeckCards() async {
        ...
        for (cardId, addedCount) in pending {
